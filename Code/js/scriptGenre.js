  ...
  const videos = Object.entries(data).map(([url, valeurs]) => ({
    ...
    function getUserDataRadar(user, selectedCategories) {
      ...
      window.updateCharts = function updateCharts() {
        const selectedUsers = utilisateurs.filter((_, i) => listeCourbesAffichées[i]);
        ...
        const radarData = selectedUsers.map((user, i) => ({
          ...
          color: couleursUtilisateurs[utilisateurs.indexOf(user)]
        }));
      ...
      function drawRadarChart(id, data, options) {
        ...
        if (data.length === 0) {
          ...
          allAxis = data[0].data.map(d => d.axis);
        ...
        axisGrid.selectAll(".level-label")
          ...
          .attr("dy", "-0.3em")
          .text(function (d) {
            if (cfg.maxValue === -Infinity){
              return "";
            }else{
              return `${Math.round(cfg.maxValue / cfg.levels * d)} vidéos`;
            }
          } );
